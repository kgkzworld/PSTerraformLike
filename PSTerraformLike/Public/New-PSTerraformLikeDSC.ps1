#region New-PSTerraformLikeDSC (Function)
Function New-PSTerraformLikeDSC
{
<#
    .SYNOPSIS
        New-PSTerraformLikeDSC will create a new PSTerraformLike DSC template.
    ...
    .PARAMETER Path
        ...
        Notes: If you do not select a path, the file will be created in the current directory and the current date will be used as the filename
                Example: .\DSC_11.29.2023_6.35.39_PM.YAML
        Alias:
        ValidateSet:
    ...
    .EXAMPLE
        Command: New-PSTerraformLikeDSC
        Description: This will create a new PowerShell DSC template in the current directory with the current date as the filename
        Notes:
        Output:

    .EXAMPLE
        Command: New-PSTerraformLikeDSC -Path 'C:\Temp\MyDSC.yaml'
        Description: This will create a new PowerShell DSC template in the C:\Temp directory with the filename of MyDSC.yaml
        Notes:
        Output:

    .EXAMPLE
        Command: New-PSTerraformLikeDSC -Help
        Description: Call Help Information
        Notes: If Help / WalkThrough is setup as a parameter, this script will be called to setup the Dynamic Help Menu if not the normal Get-Help will be called with the -Full parameter

    .EXAMPLE
        Command: New-PSTerraformLikeDSC -WalkThrough
...
#>
    ...
    [cmdletbinding()]
    [Alias('New-TFLDSC','TFLNDSC')]
    #region Parameters
    Param
    (
        [Parameter(Position = 0)]
        [String]$Path = $('.\DSC_{0}.yaml' -f $(Get-Date).ToString() -replace '/|:','.' -replace '\s+','_'),
            ...
            #region Main
                ...
                #region Main Code
                    #region Config Properties
                        $Config = @{}
                        $Config.Title = 'DSC Template Title'
                        $Config.Description = 'DSC Template Description'
                    ...
                    #region Testing Properties
                        ...
                        $Testing.LoopTimer = 0
                        $Testing.LoopCount = 0
                        $Testing.RemediationTest = 'Write-Host "Remediation Test"
                        Write-Host "Remediation Test2"'
                        $Testing.PesterTest = 'Write-Host "Pester Test"
                        Write-Host "Pester Test2"'
